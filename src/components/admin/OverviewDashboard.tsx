...
function AnimatedCard({ card, index, onNavigate }: { card: any; index: number; onNavigate: (s: string) => void }) {
  ...
  return (
    <button
      ...
      className={`group p-5 rounded-2xl border border-white/[0.05] hover:border-white/[0.1] transition-all duration-500 text-left relative overflow-hidden ${
    ...
    >
      ...
      <div className="relative">
        <div className="flex items-center justify-between mb-4">
          ...
          <FiArrowUpRight className="w-4 h-4 text-slate-600 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300" />
        </div>

        <p className="text-3xl font-black text-white mb-1 tabular-nums">{card.count}</p>
...
export default function OverviewDashboard({ data, sectionMeta, onNavigate }: {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome banner */}
      <div className={`relative p-6 rounded-2xl border border-white/[0.05] overflow-hidden transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
        ...
        <div className="relative flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1.5">Manage your portfolio content from one place</p>
          </div>
          <div className="hidden sm:flex items-center gap-2 text-xs text-slate-500 bg-white/[0.03] px-4 py-2 rounded-xl border border-white/[0.05]">
      ...
      {/* Quick actions */}
      <div className={`relative p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] transition-all duration-700 delay-500 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {["hero", "projects", "skills", "socialLinks"].map((s, idx) => (
            <button key={s} onClick={() => onNavigate(s)}
              className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.05] hover:border-white/[0.08] transition-all duration-300 text-left group relative overflow-hidden">
              {/* Hover glow */}
      ...
      {(() => {
        ...
        return settings ? (
          <div className={`relative p-5 rounded-2xl bg-white/[0.02] border border-white/[0.05] transition-all duration-700 delay-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
            <h3 className="text-white text-sm font-semibold mb-4">Site Information</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="px-4 py-3.5 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:border-white/[0.08] transition-colors duration-300">
